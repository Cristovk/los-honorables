...
import { convertXmlToJson, fetchAndConvertXml } from "../../utils/xmlToJson.ts";
...
//     console.log(url);
//     const data = await fetchAndProcessXml(url, "Votos");
...
router.get("/votacionesXProyectoLey", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log(url);
    const data =await fetchAndConvertXml(url);
    res.status(200).json(data);
  } catch (error: any) {
    res
      .status(500)
